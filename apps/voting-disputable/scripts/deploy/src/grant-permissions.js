...
async function grantPermission(acl, to, app, roleName, options = {}) {
  const web3 = getWeb3()
  const { tokenManager, voting } = options
  ...
  const grantPermissionTx = { data: forwardData, to: tokenManager, ...app.constructor.class_defaults }
  ...
  const voteTx = { data: voteData, to: voting, ...app.constructor.class_defaults }
